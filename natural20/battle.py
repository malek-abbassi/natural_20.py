...
from natural20.actions.attack_action import (
    AttackAction,
...
class Battle:
    def __init__(
        ...
        session: Session,
        maps: Map,
        standard_controller=None,
        animation_log_enabled=False,
    ):
        ...
        self.opposing_groups = {"a": ["b"], "b": ["a"], "c": ["c"]}
    ...
    def add(
        ...
        index=0,
        token=None,
        custom_initiative=None,
        add_to_initiative=False,
    ):
        ...
        if add_to_initiative:
            ...
            self.combat_order = sorted(
                self.combat_order,
                key=lambda a: self.entities[a]["initiative"],
                reverse=True,
            )
    ...
    def start(self, combat_order=None, custom_initiative=None):
        ...
        if isinstance(custom_initiative, list):
            ...
            _combat_order = [
                [entity, v] for entity, v in self.entities.items() if not entity.dead()
            ]
        ...
        self.event_manager.received_event(
            {
                ...
                "combat_order": [
                    [e, self.entities[e]["initiative"]] for e in self.combat_order
                ],
    ...
    def current_turn(self) -> Entity:
        ...
        if self.current_turn_index >= len(self.combat_order):
            raise Exception(
                f"current_turn_index out of bounds {self.current_turn_index} >= {len(self.combat_order)}"
            )
    ...
    def eval_legendary_action(self):
        for entity in self.combat_order:
            ...
            if entity.npc() and len(entity.legendary_actions) > 0:
                ...
                if controller:
                    action = controller.legendary_action_listener(
                        self,
                        self.session,
                        entity,
                        self.map_for(entity),
    ...
    def next_turn(self, max_rounds=None):
        ...
        if self.started and self.battle_ends():
            self.session.event_manager.received_event(
                {"source": self, "event": "end_of_combat"}
            )
            self.started = False
            # print('tpk')
            return "tpk"
    ...
    def entity_state_for(self, entity):
        ...
        if entity_state is None:
            ...
            _entity = self.entity_by_uid(uid)
    ...
    def opponents_of(self, entity):
        return [
            k
            for k in (list(self.entities.keys()) + self.late_comers)
            if not k.dead() and self.opposing(k, entity)
        ]
    ...
    def allies_of(self, entity):
        return [
            k
            for k in (list(self.entities.keys()) + self.late_comers)
            if not k.dead() and self.allies(k, entity)
        ]
    ...
    def enemy_in_melee_range(self, source, exclude=None, source_pos=None):
        ...
        for object in objects_around_me:
            ...
            if self.opposing(source, object) and (
                map_for_source.distance(source, object, entity_1_pos=source_pos)
                <= (object.melee_distance() / map_for_source.feet_per_grid)
            ):
                return True
    ...
    def resolve_action(self, source, action_type, opts=None):
        ...
        action = next(
            (
                act
                for act in source.available_actions(self.session, self)
        ...
        opts["battle"] = self
        return (
            action.resolve(self.session, self.map_for(action.source), opts)
            if action
            else None
        )
                ...
                if self.animation_log_enabled:
                    if item.get("perception_targets"):
                        ...
                        self.animation_log.append(
                            {
                                "type": "perception",
                                "targets": [p.entity_uid for p in perception_targets],
                            }
                        )

        if action.action_type == "move":
            self.trigger_event(
                "movement", action.source, {"move_path": action.move_path}
            )
            if self.animation_log_enabled:
                if len(self.animation_log) == 0:
                    self.animation_log.append(
                        [
                            action.source.entity_uid,
                            [self.entity_or_object_pos(action.source)],
                            None,
                        ]
                    )
                self.animation_log.append(
                    [action.source.entity_uid, action.move_path, None]
                )
        elif action.action_type == "attack":
            if self.animation_log_enabled and len(self.animation_log) > 0:
                self.animation_log[-1][2] = {
                    "target": action.target.entity_uid,
                    "type": "attack",
            ...
            if (
                self.animation_log_enabled
                and action.target
                and action.avg_damage(self) > 0
            ):
                if len(self.animation_log) == 0:
                    self.animation_log.append(
                        [
    ...
    def trigger_event(self, event, source, opt=None):
        ...
        if event in self.battle_field_events:
            for object, handler in self.battle_field_events[event].items():
                object.__getattribute__(handler)(
                    self, source, {**opt, "ui_controller": self.controller_for(source)}
                )
        if self.maps and self.map_for(source):
            self.map_for(source).activate_map_triggers(
                event, source, {**opt, "ui_controller": self.controller_for(source)}
            )
    ...
    def can_see(
        ...
        if entity1 == entity2:
            return True
        if not map1.can_see(
            entity1, entity2, entity_1_pos=entity_1_pos, entity_2_pos=entity_2_pos
        ):
            return False

        return True

    def valid_targets_for(
        self,
        entity,
        action,
        target_types=None,
        ...
        if target_types is None:
            if isinstance(action, AttackAction) or isinstance(
                action, TwoWeaponAttackAction
            ):
        ...
        active_perception = (
            ...
            else self.active_perception_for(entity)
        )
        target_types = (
            [target_type.lower() for target_type in target_types]
            if target_types
            else ["enemies"]
        )
        ...
        for k, prop in self.entities.items():
            ...
            if "self" not in target_types and k == entity:
                continue
            if (
                "allies" not in target_types
                and prop["group"] == entity_group
                and k != entity
            ):
                continue
            ...
            if k.hp() is None:
                continue
            if (
                "ignore_los" not in target_types
                and not entity == k
                and not self.can_see(entity, k, active_perception=active_perception)
            ):
                continue
            if (
                self.maps
                and self.map_for(k).distance(k, entity) * self.map_for(k).feet_per_grid
                > attack_range
            ):
                continue
        ...
        if include_objects:
            ...
            for object, _position in _map.interactable_objects.items():
                if object.dead():
                    continue
                if "ignore_los" not in target_types and not self.can_see(
                    entity, object, active_perception=active_perception
                ):
                    continue
    ...
    def ally_within_enemy_melee_range(
        self, source, target, exclude=None, source_pos=None
    ):
        ...
        for object, _ in objects_around_me.items():
            ...
            if self.allies(source, object) and (
                _map.distance(target, object, entity_1_pos=source_pos)
                <= (object.melee_distance() / _map.feet_per_grid)
            ):
                return True
    ...
    def trigger_opportunity_attack(self, entity, target, cur_x, cur_y, action=None):
        event = {"target": target, "position": [cur_x, cur_y]}
        if action is None:
            action = entity.trigger_event(
                "opportunity_attack", self, self.session, self.map_for(entity), event
            )
            # check if action is a generator due to a yield
            if hasattr(action, "send"):
                return action
